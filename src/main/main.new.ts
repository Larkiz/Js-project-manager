...
import projectsAction from './listeners/projects';
...
//     })
//     .then((res) => {
//       event.reply('getDir', res.filePaths[0]);
//     }),
// );
...
ipcMain.on('startProject', (event, path, _id) => {
  ...
  // Обработка вывода из дочернего процесса
  serverProcess.stdout.on('data', (data) => {
    console.log(`stdout: ${data}`);
    event.reply('projectStatus', { id: _id, running: true });
  });
  ipcMain.once(`stopProject_${_id}`, (event, _id) => {
    serverProcess.kill();
    event.reply('projectStatus', { id: _id, running: false });

    // ipcMain.removeAllListeners(`stopProject_${_id}`);
  });
  // Обработка ошибок
